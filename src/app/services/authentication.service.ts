...
export class AuthenticationService {
  ...
  logout(){
    ...
    this.storage.get("USER_DATA").then((data)=>{
      ...
      this.dbserviceService.updateSesionData(data)
  ...
  login(login:any){
    ...
    this.dbserviceService.getSesionData(login)
    .then((data)=>{ // Si se ejecuto correctamente la consulta
      if(data===undefined){ // Si es undefined es por que no retorno firmas
        ...
        this.dbserviceService.updateSesionData(data) // Y se actualiza la sesión
